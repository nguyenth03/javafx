...
public class AccountEntity {
    ...
    public static ObservableList<Account> SearchByUID(String UID) {
        ...
        return null;
            ...
            @FXML
    public static void Add(SignUp sign) throws IOException {
        String sql = "INSERT INTO `accounts` (`username`, `password`, `full_name`, `email`, `dob`, `mobile`) VALUES (?, ?, ?,?,?, ?);";
        try {
            LocalDate dateBorrow = sign.getInp_date_birth().getValue();
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            String date1 = dateBorrow.format(formatter);
            connection = JDBCConnect.getJDBCConnection();
            preparedStatement = connection.prepareCall(sql);
            preparedStatement.setString(1, sign.getInp_username().getText());
            preparedStatement.setString(2, sign.getInp_password().getText());
            preparedStatement.setString(3, sign.getInp_fullname().getText());
            preparedStatement.setString(4, sign.getInp_email().getText());
            preparedStatement.setString(5, date1);
            preparedStatement.setString(6, sign.getInp_mobile().getText());
            preparedStatement.executeUpdate();
            System.out.println("ABC");
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Test Connection");
            alert.setHeaderText("AcountManager");
